...
/**
 * GraphQL interceptor that logs each incoming request for observability and
 * attack-pattern detection.
 *
 * <p>
 * Fires before every GraphQL execution and emits a single structured
 * {@code INFO} log line
 * containing the caller's effective remote IP, the request URI, the GraphQL
 * document, and any
 * variables. The log format uses {@code key="value"} pairs so Loki/LogQL can
 ...
 * Documents and variable maps longer than {@value #MAX_PAYLOAD_LENGTH}
...
public class GraphQlRequestLoggingInterceptor implements WebGraphQlInterceptor {
  ...
  /**
   * Maximum number of characters logged for document or variables before
   * truncation.
   */
   ...
   *
   * @param request the incoming GraphQL request, including HTTP headers and the
   *                parsed document
  ...
  /**
   * Truncates a string to {@value #MAX_PAYLOAD_LENGTH} characters, appending a
   * marker when cut.
   *
   * @param value the string to truncate; may be {@code null}
   * @return the original value if short enough, a truncated version otherwise, or
   *         {@code null}
   */
